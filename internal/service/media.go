...
func (s *MediaService) GenerateSnapshots(videoPath string, outputDir string) ([]string, error) {
	...
	// -vf "fps=1/5" captures one frame every 5 seconds
	cmd := exec.Command("ffmpeg", "-i", videoPath, "-vf", "fps=1/5", filepath.Join(outputDir, "thumb%03d.jpg"))
...
func (s *MediaService) ExtractAudio(videoPath string, outputPath string) error {
	cmd := exec.Command("ffmpeg", "-i", videoPath, "-vn", "-acodec", "libmp3lame", "-y", outputPath)
	if err := cmd.Run(); err != nil {
